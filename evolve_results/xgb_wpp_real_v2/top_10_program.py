...
PARAMS = {
    ...
    "eval_metric": "logloss",
    "max_depth": 7,  # Increased slightly from 6 to 7 (within 5-8 range)
    "learning_rate": 0.035, # Increased slightly from 0.03 (within 0.02-0.05 range)
    "n_estimators": 300, # Kept same, early stopping will manage
    "subsample": 0.8, # Kept same
    "colsample_bytree": 0.85, # Increased slightly from 0.8 (within 0.6-0.9 range)
    "min_child_weight": 1,
    "gamma": 0.05, # Added minimal split regularization (within 0-1 range)
    "reg_alpha": 0.1, # Added minimal L1 regularization (within 0-2 range)
    "reg_lambda": 1.0,
    "scale_pos_weight": 1.0,
    "max_delta_step": 0,
    "random_state": 42,
    # Monotonic constraint: net_wins (index 12) must increase prediction.
    # Added negative constraint for vol_20d (index 9) as per guidance (high vol = bearish).
